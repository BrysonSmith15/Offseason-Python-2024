...
class PPHolonomicDriveController(PathFollowingController):
    ...
    _mpsToRps: float
    _translationError: Translation2d = Translation2d(0, 0)

    _isEnabled: bool = True
    _rotationTargetOverride: Union[Callable[[],
                                            Union[Rotation2d, None]], None] = None
    ...
    def __init__(self, translation_constants: PIDConstants, rotation_constants: PIDConstants, max_module_speed: float,
        ...
        self._xController = PIDController(translation_constants.kP, translation_constants.kI, translation_constants.kD,
                                          period)
        self._xController.setIntegratorRange(
            -translation_constants.iZone, translation_constants.iZone)

        self._yController = PIDController(translation_constants.kP, translation_constants.kI, translation_constants.kD,
                                          period)
        self._yController.setIntegratorRange(
            -translation_constants.iZone, translation_constants.iZone)
        ...
        self._rotationController.setIntegratorRange(
            -rotation_constants.iZone, rotation_constants.iZone)
    ...
    def calculateRobotRelativeSpeeds(self, current_pose: Pose2d, target_state: State) -> ChassisSpeeds:
        ...
        xFeedback = self._xController.calculate(
            current_pose.X(), target_state.positionMeters.X())
        yFeedback = self._yController.calculate(
            current_pose.Y(), target_state.positionMeters.Y())
        ...
        if math.isfinite(maxAngVel):
            ...
            maxAngVelModule = max(
                0.0, self._maxModuleSpeed - target_state.velocityMps) * self._mpsToRps
    ...
    def reset(self, current_pose: Pose2d, current_speeds: ChassisSpeeds) -> None:
        ...
        self._rotationController.reset(
            current_pose.rotation().radians(), current_speeds.omega)
