    ...
    def manual_control(
        ...
        *,
        ...
        out = FunctionalCommand(
            ...
            onExecute=lambda: self.set_motors(
                speed_control()
                if not (top_condition() and speed_control() >= 0)
                or (bottom_condition() and speed_control() <= 0)
                else 0
            ),
            onEnd=lambda _interrupted: self.set_motors(0),
            isFinished=lambda: (top_condition() and speed_control() >= 0)
            or (bottom_condition() and speed_control() <= 0),
        )
        out.addRequirements(self)
        return out
